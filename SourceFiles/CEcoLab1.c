...
int16_t ECOCALLMETHOD CEcoLab1_poly(/* in */ struct IEcoLab1* me, /* in */ void* roots_data, size_t roots_count,
    ...
    int res_init = initOnes(coefs_data);
    if (res_init != 0) {
        return -2;
    }
    ...
    char* buffer = pCMe->m_pIMem->pVTbl->Alloc(pCMe->m_pIMem, coefs_count * type_size);
    for (int i = 1; i < coefs_count; ++i) {
        /* Записываем в буфер текущие коэффициенты */
		for (int j = 0; j < coefs_count; ++j) {
            copy_element(coef_arr + j * type_size, buffer + j * type_size, type_size);
		}
        ...
        for(int j = 1; j < i + 1; ++j){
            ...
            int mult_res = mult(root_arr + (i - 1) * type_size, buffer + (j - 1) * type_size, temp1);
            if (mult_res != 0) {
                return -2;
            }
            int sub_res = sub(coef_arr + j * type_size, (void*)temp1, (void*)(coef_arr + j * type_size));
            if (sub_res != 0) {
                return -2;
            }
